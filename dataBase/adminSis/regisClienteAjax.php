<?php //inicio el php
	...
	// de buscaRepetido se responde a regisEst.php mediante el echo
		if(buscaRepetido($tipodoc,$numdocumento,$apellidos,$conexion)==1){  // si el resultado de la funcion es igual a 1
			...
			$sql="INSERT into clientes (nomCliente,apeCliente,emailCliente,fechaNacCliente,idTipoDoc,numDocCliente,idEstCivil,celCliente,telCliente,direCliente)
				values ('$nombres','$apellidos','$email','$fechanac','$tipodoc','$numdocumento','$estcivil','$celular','$telefono','$direccion')";
		...
		*/
		...
		function buscaRepetido($tipo,$numdocu,$ape,$conexion){  
			...
			$sql="SELECT * from clientes where idTipoDoc='$tipo' and numDocCliente='$numdocu' and apeCliente='$ape'"; 
