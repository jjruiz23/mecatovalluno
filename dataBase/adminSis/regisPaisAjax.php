<?php //inicio el php
	...
	$conexion=conexion();  //creo variable $conexion y le asigno los valores de la funcion conexion importada por conexion.hp
		...
		$nuevoPais=$_POST['nuevoPais']; // creo variable $tipodoc y le asigno los datos recibidos desde regisEst.php
		
	// de buscaRepetido se responde a regisEst.php mediante el echo
		if(buscaRepetido($nuevoPais,$conexion)==1){  // si el resultado de la funcion es igual a 1
			...
			$sql="INSERT into pais (nomPais) values ('$nuevoPais')";
		...
		function buscaRepetido($nvPais,$conexion){  
			...
			$sql="SELECT * from pais where nomPais='$nvPais'"; 
