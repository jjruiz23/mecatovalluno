<?php //inicio el php
	...
	$conexion=conexion();  //creo variable $conexion y le asigno los valores de la funcion conexion importada por conexion.hp
		...
		$nuevaCiudad=$_POST['nuevaCiudad']; // creo variable $tipodoc y le asigno los datos recibidos desde regisEst.php
		$idPais=$_POST['idPais'];

	// de buscaRepetido se responde a regisEst.php mediante el echo
		if(buscaRepetido($nuevaCiudad,$idPais,$conexion)==1){  // si el resultado de la funcion es igual a 1
			...
			$sql="INSERT into ciudad (nomCiudad,idPais) values ('$nuevaCiudad','$idPais')";
		...
		function buscaRepetido($nvCiudad,$nvIdPais,$conexion){  
			...
			$sql="SELECT * from pais where nomCiudad='$nvCiudad' and idPais='$nvIdPais'"; 
